...
def movement1(request):
    ...
    result = {'dailynum' : daily, 'citycov' : citycovid}
...
def hospi(request):
    stmt = "select * from project_covidhospital"
    hospi = pd.read_sql(stmt, conn).to_dict('records')
    result = {"hospital" : hospi}
    return render(request, "hospi.html", result)